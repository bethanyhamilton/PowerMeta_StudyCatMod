...
# dat_kjN_math_study <- dat_kjN_math_study |> filter(kj <= N - 2)
# 
# dat_kjN_math_study |> 
#   summarise(
#     num_studies = length(unique(CitationID))
...
dat_kjN_math <- 
  math_dat |>  
  dplyr::select(CitationID, cluster_id, kj, N = N_ess, sigma_j_sq) |> 
  # Excluding effective sample sizes larger than 500 
  # and studies with more than 20 outcomes
  filter(N < 500, kj < 20) |> 
  as.data.frame()
...
saveRDS(dat_kjN_math, "SimFunctions/dat_kjN_mathdat_alt.rds")


# dat_kjN_math$group <- "sample"
# dat_kjN_math_study$group <- "study"
# 
# test <- bind_rows(dat_kjN_math, dat_kjN_math_study)
# 
# 
# test <- test |> mutate(
#   N.alt = 4/sigma_j_sq,
...
# ) |> 
#   rename(N.used = N,
#          sigma_j_sq.used = sigma_j_sq)
#   
# 
# ggplot(test, aes(x = N_alt, fill =group )) +
#   geom_density(alpha=.5) +
#   labs(title = "Distributions across runs") +
#   labs(y="Density") +
#   labs(x="N")
# 
# 
# ggplot(test, aes(x = sigma_j_sq, fill =group )) +
#   geom_density(alpha=.5) +
#   labs(title = "Distributions across runs") +
#   labs(y="Density") +
#   labs(x="N")
# 
# ggplot(test, aes(x = sigma_j_sq_alt, fill =group )) +
#   geom_density(alpha=.5) +
#   labs(title = "Distributions across runs") +
#   labs(y="Density") +
#   labs(x="N")
...
# test_long <- test %>%
#   pivot_longer(
#     cols = c(sigma_j_sq.used, N.used, sigma_j_sq.alt, N.alt),
#     names_to = c(".value", "time"),
#     names_sep = "\\."
#   ) |>  filter(group == "sample") 
# 
# #|#> unite("group2", c(group, time), sep = ".")
# 
# ggplot(test_long, aes(x = sigma_j_sq, fill =time )) +
#   geom_density(alpha=.5) +
#   labs(title = "Distributions across runs") +
#   labs(y="Density") +
#   labs(x="sigma_j_sq")
# 
# ggplot(test_long, aes(x = N, fill =time )) +
#   geom_density(alpha=.5) +
#   labs(title = "Distributions across runs") +
#   labs(y="Density") +
#   labs(x="N")
